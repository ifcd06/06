...
$query = "SELECT productos.*, fotos.nombre AS nombrefoto  
FROM productos INNER JOIN fotos 
WHERE productos.id = fotos.idproducto AND productos.id = $id
GROUP BY productos.id ";
$resultset = mysqli_query($conexion, $query);
$row = mysqli_fetch_assoc($resultset);

//Si se ha pulsado añadir al carrito debemos hacerlo
if (isset($_REQUEST["agregarproducto"]))
{
  $cantidad = sanitizar($conexion, $_REQUEST["cantidad"]);
  $carrito->agregarproducto($id, $row["nombre"], $row["nombrefoto"], $row["precio"], $cantidad);
  //print "<script>window.location.href='?modulo=detalleproducto&id=$_REQUEST[id]'</script>";
  print "<meta http-equiv=\"refresh\" content=\"0; url=index.php?modulo=detalleproducto&id=$id\" />  ";
}
/*print "<pre>";
print_r($carrito);
print "</pre>";*/
        ...
        <div class="col-12 col-sm-6">
          <h3 class="my-3"><?php print $row["descripcion"] ?></p>
            ...
            <div class="bg-gray py-2 px-3 mt-4">
              ...
              <h4 class="mt-0">
                <small><?php print(($row["existencias"] == 0) ? "No disponible" : "Disponible") ?> </small>
              </h4>
            </div>
            <?php if ($row["existencias"] != 0) { ?>
            <form action="index.php?modulo=detalleproducto&id=<?php print $id ?>" method="post">
              <div class="form-group">
                <label for="cantidad">Cantidad: </label>
                <input type="number" name="cantidad" class="form-control" placeholder="" aria-describedby="helpId" value="1">
              </div>
              <div class="mt-4">
                <button type="submit" class="btn btn-primary btn-lg btn-flat" name="agregarproducto">
                  <i class="fas fa-cart-plus fa-lg mr-2"></i>
                  Añadir al carrito
                </button>
                
            <?php } ?>
            <a href="index.php?modulo=productos" class="btn btn-danger btn-lg btn-flat">
